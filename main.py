...
logging.basicConfig(
    format="[%(levelname)s]: %(message)s", level=logging.DEBUG if config['debug'] else logging.INFO
)
...
async def fetch_data():
    ...
    while True:
        ...
        hp.pop()
        strength_offset = int((100 - hp[0]) * config['dglab']['strength_per_hp'])
        ...
        if hp[0] < hp[1]:
            logging.info(f"You're injured! Current HP {hp[0]}. Setting strength to {BASE_STRENGTH + strength_offset}")
        if hp[0] > hp[1]:
            logging.info(f"You're healed! Current HP {hp[0]}. Setting strength to {BASE_STRENGTH + strength_offset}")
        # print(hp)
